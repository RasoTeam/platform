...
describe Company do
  ...
  before do
    @company = Company.new(name: "test company inc", tag: "tci", logo_url: "http://somelogo.com", address: "company address, 123", state: 0, nif: 123456789 )
  end
  ...
  describe "when tag in not present" do 
    before { @company.tag = " " }
    it { should_not be_valid }
  end
  ...
  describe "when tag is too short" do
    before { @company.tag = "a" * 2 }
    it { should_not be_valid }
  end

  describe "when tag is too long" do
    before { @company.tag = "a" * 21 }
    it { should_not be_valid }
  end

  describe "when tag format is invalid" do
    it "should be invalid" do
      tags = %w[asd-+ *A* AxS-# -asd _sad]
      tags.each do |invalid_tag|
        @company.tag = invalid_tag
